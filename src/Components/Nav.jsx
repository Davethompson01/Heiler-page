...
const Nav = ({ handleScrollToAbout }) => {
  const maps = [
    ...
    { h1: "Find Doctor", path: "http://app.heiler.health/onboarding" },
  ...
  useEffect(() => {
    const activeIndex = maps.findIndex(
      (map) =>
        map.path === location.pathname ||
        (map.path === "/#Service" && location.pathname === "/")
    );
    setActiveLinks(
      maps.reduce(
        (acc, _, index) => ({ ...acc, [index]: index === activeIndex }),
        {}
      )
    );
